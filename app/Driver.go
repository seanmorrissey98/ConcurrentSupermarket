...
func (c *Checkout) ProcessCheckout() {
	for {
		...
		for _, p := range products {
			time.Sleep(time.Millisecond * time.Duration(p.time*500*c.speed*ageMultiplier))
			atomic.AddInt64(&c.processedProducts, 1)
			atomic.AddInt64(&c.processedProductsTime, int64(p.time*500*c.speed))
		}
...
func (c *Customer) Shop(readyForCheckoutChan chan int) {
	...
	for {
		...
		p := NewProduct()
		time.Sleep(time.Millisecond * time.Duration(p.time*200*speedMultiplier))
		c.trolley.AddProductToTrolley(p)
		c.shopTime += int64(p.time * 200)
		if c.trolley.IsFull() {
			break
		}
